...
namespace Mediapipe.Unity.HandTracking
{
	...
	public event Action<IList<NormalizedLandmarkList>> OnHandLandmarkUpdated;
	public event Action<IList<ClassificationList>> OnHandednessUpdated;
	...
	private void OnHandLandmarksOutput(object stream, OutputEventArgs<List<NormalizedLandmarkList>> eventArgs)
	{
	    ...
	    OnHandLandmarkUpdated?.Invoke(eventArgs.value);
	}
	...
	{
	    //_handLandmarksAnnotationController.DrawLater(eventArgs.value);
	    OnHandednessUpdated?.Invoke(eventArgs.value);
	}
    }
}
